n_observations <- function(package = NULL,
  ...
  camtraptor:::check_package(package, datapkg, "n_observations")
  ...
  camtraptor:::check_value(sex_value, unique(package$data$observation$sex), "sex")
  camtraptor:::check_value(life_stage, unique(package$data$observation$lifeStage), "lifeStage")
  ...
  deployments_no_obs <- camtraptor:::get_dep_no_obs(
    package,
  ...
  # Generate the date sequences for each deployment
  deployments_expanded <- deployments %>%
    rowwise() %>%
    mutate(date_range = list(seq(start, end, by = "day"))) %>%
    unnest(date_range) %>%
    mutate(date = as.Date(date_range)) %>%
    select(deploymentID, date)
  
  # Expand species combinations
  species_list <- unique(c(unique(observations$scientificName), species))
  
  combinations_dep_species_dates <- deployments_expanded %>%
    tidyr::crossing(scientificName = species_list) %>%
    select(deploymentID, scientificName, date)
